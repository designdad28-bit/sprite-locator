...
const LAYERS = JSON.parse(process.env.WATER_LAYERS || "null") ?? [
  ...
  // Recoloured by request to the light band's #1395d8 (Fortnite's own is
  // #0f5eb3): both bands and the map surround share one blue. The band is
  // kept so it can take its own colour again.
